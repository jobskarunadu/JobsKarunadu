...
export const api = {
  ...
  async adminDeleteAllSampleJobs(token: string): Promise<{ success: boolean; removedCount: number; message: string }> {
    const res = await fetch(`${API_BASE}/admin/jobs/bulk/sample-jobs`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.error || 'Failed to delete sample jobs');
    }
    return res.json();
  },

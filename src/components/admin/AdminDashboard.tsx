...
export const AdminDashboard: React.FC = () => {
  ...
  // Bulk Clear All Sample / Demo Listings
  const handleClearAllSampleJobs = async () => {
    if (!token) return;
    setIsClearingSamples(true);
    try {
      const res = await api.adminDeleteAllSampleJobs(token);
      showNotification(res.message || 'All demo listings were removed.');
      setConfirmClearSamplesOpen(false);
      await loadJobs();
      const newStats = await api.adminGetStats(token);
      setStats(newStats);
    } catch (err: any) {
      showNotification(err.message || 'Failed to clear sample jobs');
    } finally {
  ...
  return (
    <div id="admin-dashboard-container" className="min-h-screen bg-slate-100/70 pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                id="btn-admin-clear-samples"
                onClick={() => setConfirmClearSamplesOpen(true)}
                className="px-3 py-2 bg-slate-800 hover:bg-rose-950/80 text-slate-300 hover:text-rose-300 border border-slate-700 text-xs font-medium rounded-xl transition-all flex items-center gap-1.5 active:scale-95"
                title="Remove all pre-loaded sample/demo listings"
              >
                <Trash2 className="w-3.5 h-3.5 text-rose-400" />
                <span>Clear Demo Jobs</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* CONFIRM CLEAR ALL SAMPLE JOBS MODAL */}
      {confirmClearSamplesOpen && (
        <div 
          id="admin-clear-samples-modal-backdrop"
          className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in"
          onClick={() => !isClearingSamples && setConfirmClearSamplesOpen(false)}
        >
          <div 
            id="admin-clear-samples-modal-box"
            className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 text-left"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center gap-3 text-amber-600 mb-3">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-base">Remove All Demo Listings?</h3>
                <p className="text-xs text-slate-500">Clean slate for live production</p>
              </div>
            </div>

            <p className="text-xs text-slate-700 mb-6 bg-slate-50 p-3.5 rounded-xl border border-slate-200 leading-relaxed">
              This will permanently remove all pre-loaded sample job listings and demo gigs from the database. 
              <strong className="text-slate-900 block mt-1">Any new jobs and gigs you created will be safely preserved and stay permanent.</strong>
            </p>
            ...
            <div className="flex items-center justify-end gap-2.5">
              <button
                ...
                onClick={() => setConfirmClearSamplesOpen(false)}
                className="px-4 py-2 text-xs font-semibold text-slate-600 hover:text-slate-800 rounded-lg hover:bg-slate-100 transition-colors"
              >
                Cancel
              </button>
              ...
              <button
                id="btn-confirm-clear-samples"
                type="button"
                disabled={isClearingSamples}
                onClick={handleClearAllSampleJobs}
                className="px-5 py-2 bg-rose-600 hover:bg-rose-700 disabled:bg-rose-400 text-white text-xs font-bold rounded-xl shadow-md shadow-rose-600/20 transition-all flex items-center gap-1.5 active:scale-95"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{isClearingSamples ? 'Removing...' : 'Yes, Remove Demo Jobs'}</span>
              </button>
            </div>
          </div>
        </div>

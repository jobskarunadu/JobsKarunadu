...
class Database {
  ...
  private loadData(): DatabaseSchema {
    try {
      if (fs.existsSync(DB_FILE)) {
        ...
        if (parsed && Array.isArray(parsed.jobs)) {
          // Keep categories and sources present if missing
          if (!parsed.categories || !Array.isArray(parsed.categories) || parsed.categories.length === 0) {
            parsed.categories = INITIAL_CATEGORIES;
          }
          if (!parsed.sources || !Array.isArray(parsed.sources) || parsed.sources.length === 0) {
            parsed.sources = INITIAL_SOURCES;
          }
          if (!parsed.reports || !Array.isArray(parsed.reports)) {
            parsed.reports = [];
          }
          // Do NOT re-add deleted sample jobs! Whatever is in parsed.jobs is the true saved state.
          return parsed;
        }
      }
    } catch (err) {
    ...
    // Default state ONLY if db.json never existed at all
  ...
  // Delete all sample/demo jobs
  public deleteAllSampleJobs(): number {
    const sampleIds = new Set(SAMPLE_JOBS.map(j => j.id));
    const initialCount = this.data.jobs.length;
    this.data.jobs = this.data.jobs.filter(j => 
      !sampleIds.has(j.id) && 
      !j.description?.includes('[SAMPLE LISTING') &&
      !j.title?.includes('(Archived/Expired Demo)')
    );
    const removedCount = initialCount - this.data.jobs.length;
    if (removedCount > 0) {
      this.saveDataImmediately(this.data);
    }
    return removedCount;
